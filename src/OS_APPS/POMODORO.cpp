...
static constexpr const char* TAG = "POMODORO";
...
void POMODORO_INIT() {
  CurrentAppState = POMODORO;
  currentPomodoroState = POMODORO_IDLE;
  timerRunning = false;
  EINK().forceSlowFullUpdate(true);
  loadPomodoroCount();
  newState = true;
  ESP_LOGI(TAG, "Pomodoro app initialized");
}

void processKB_POMODORO() {
  ...
  switch (currentPomodoroState) {
    case POMODORO_IDLE:
      if (newState) {
        ...
        EINK().drawStatusBar("Select Option (1-3), Reset (R), Back (ESC)");
        EINK().refresh();
      }
      break;
